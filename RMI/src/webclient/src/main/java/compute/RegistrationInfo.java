...
public class RegistrationInfo implements Serializable
{
    ...
    public static ArrayList<RegistrationInfo> jsonConvertList(String regInfo) {

        ArrayList<RegistrationInfo> registrationInfos = new ArrayList<>();
        JSONArray array = new JSONArray(regInfo);

        for (int i = 0; i < array.length(); i++) {
            registrationInfos.add(RegistrationInfo.jsonConvertEntry(array.getJSONObject(i)));
        }

        return registrationInfos;
    }

    private static RegistrationInfo jsonConvertEntry(JSONObject jObj) {
        String username = jObj.getString("userName");
        String host = jObj.getString("host");
        int port = jObj.getInt("port");
        boolean available = jObj.getBoolean("status");

        return new RegistrationInfo(username, host, port, available);
    }
}